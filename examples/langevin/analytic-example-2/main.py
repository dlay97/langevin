...
# print(invInert.grad(meshArr[0,0]))
# print(50*'=')
...
for pid in range(nIndependentRuns):
    ...
    with lgv.stdout_redirected(to=str(pid)+'.out'):
        lgvObj = lgv.Langevin(pid,uniqueCoords,'fortran',gamma,neckVals,zz,
                              invInert(meshArr,returnFlat=True),
                              invInertGrad=invInert.grad(meshArr,returnFlat=True),
                              outputDir='logs-'+str(nRuns))
    
        #A, energyOnOuterTurningLine, dt, maxTime, scissionNeckVal, savePaths, seed
        nonArrayParams = [1000,pes(startCoords),10.**(-4),1.,0.5,True,seed]
...
ax.set(xlim=(0,3),ylim=(-1,5),
        xlabel='x',ylabel='y',title=r'Langevin vs Exact, $V(x)\sim x^2$, $M_{ij}^{-1}\sim x^2$')
ax.legend()
